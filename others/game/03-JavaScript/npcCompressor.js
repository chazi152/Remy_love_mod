...
const { npcCompressor, npcDecompressor, childCompressor, childDecompressor } = (function () {
	...
	const breastdescList = ["乳头", "微微隆起的", "含苞欲放的", "小的", "玲珑的", "适中的", "圆润的", "大的", "浑圆的", "丰满的", "巨大的", "爆裂的", "极致宏伟的"];
	...
	function npcDecompressor(passedNPC) {
		...
		const breastdesc = breastdescList[breastsize] + (breastsize > 0 ? "乳房" : "");
		const breastsdesc = breastdesc + "";
		position += 2;
		...
		if (beast) {
			...
			fullDescription = description + (!monster ? (gender === "m" ? "雄性" : "雌性") : " ") + npcType.replace("human","人类").replace("dog","狗").replace("cat","猫").replace("pig","猪").replace("wolf","狼").replace("dolphin","海豚").replace("lizard","蜥蜴").replace("bear","熊").replace("boar","野猪").replace("creature","小生物").replace("horse","马").replace("fox","狐狸").replace("hawk","鹰").replace("cow","奶牛").replace("spider","蜘蛛").replace("plant","植物").replace("sheep","羊").replace("deer","鹿").replace("bee","蜜蜂").replace("wasp","黄蜂").replace("dragon","龙").replace("ant","蚂蚁").replace("raccoon","浣熊").replace("frog","青蛙").replace("rat","老鼠").replace("slime","史莱姆").replace("slug","蛞蝓").replace("lurker","潜伏者").replace("squild","鱿鱼").replace("bull","公牛").replace("centaur","半人马").replace("harpy","哈比").replace("boy","男孩").replace("girl","女孩").replace(" ","");
		} else {
			let descType;
			let healthType;
			...
			const plant = npcType === "plant" ? "植物" : "";
			const man = pronoun === "m" ? (teen === 1 ? "男孩" : "男人") : teen === 1 ? "女孩" : "女人";
